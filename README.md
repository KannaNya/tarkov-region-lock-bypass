...
7. 默认 `DisconnectAtRaid=true`：登录、选角色、大厅和匹配阶段保持 SoftEther 连接与鉴权分流；只有本机游戏日志记录带时间戳的 `GameStarted:`，确认已经进入 Raid 后，才撤销本工具的 `/32` 路由并断开 SoftEther。当前游戏会话内不再重连。若显式设为 `false`，则使用旧的匹配/Raid 保持会话保护模式。旧配置项 `DisconnectAtMenu` 不再生效。

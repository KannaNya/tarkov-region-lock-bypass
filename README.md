...
6. 当前节点失效时，删除本工具管理的旧路由并尝试下一个；确认属于远端的失败会隔离 15 分钟，本机虚拟网卡尚未释放（SoftEther 错误 43）则只等待并有限重试，不会误伤候选节点。
7. 一轮故障转移默认最多运行 180 秒，并优先尝试不同 IP 的独立中继；同一中继的其他 SSL 端口只作为后备，不会挤占全部候选名额。
